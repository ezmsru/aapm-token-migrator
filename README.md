...
| `merge_mrs`             | `false`                                 | `true` — режим слияния (см. ниже), миграция не выполняется |
...
`merge_mrs=true` — пайплайн **не мигрирует**, а рекурсивно по всем репам группы
сливает (`PUT .../merge`) открытые MR из `feature_branch`, удаляя ветку-источник
(`should_remove_source_branch=true`). Уважает `dry_run`:

- `dry_run=true` — только показать, какие MR были бы слиты;
- `dry_run=false` — реально слить.

MR, которые не сливаются (конфликты, требуются одобрения, не пройден пайплайн,
нет прав), помечаются как ошибки в сводке, и job завершается с ненулевым кодом.

> Если включены сразу несколько режимов, приоритет: `cleanup_mrs` → `merge_mrs`
> → обычная миграция. Лишние режимы лучше не комбинировать.
